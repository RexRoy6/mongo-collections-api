...
class DetectBusiness
{
    public function handle(Request $request, Closure $next): Response
    {
        try {
            ...
            // Log error but continue (admin routes don't need business)
            \Log::error('DetectBusiness middleware error: ' . $e->getMessage());
            return $next($request);
        }
    }
    ...
    private function findBusiness($identifier)
    {
        if ($identifier['type'] === 'code') {
            return Business::where('business_code', (int)$identifier['value'])->first();
        }
